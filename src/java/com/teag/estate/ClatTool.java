...
public class ClatTool extends EstatePlanningTool {
	...
	public double calcValue(int years) {
		...
		for (Object ai : assetList) {
			...
			if (item.getAssetType() != AssetData.SECURITY)
				value += item.getValue()
						* Math.pow(1 + item.getGrowth(), years);
		}
		return value;
	}
	...
	public void buildCashFlow() {
		...
		for (i = 0; i < term; i++) {
			...
			cashflow[i][5] = -ap;
			cashflow[i][6] = cashflow[i][1] + cashflow[i][2] + cashflow[i][3]
					+ cashflow[i][4] + cashflow[i][5];
	...
	public void buildStdClat() {
		...
		for (int i = 0; i < clatTerm; i++) {
			...
			schedule[i][5] = schedule[i][0] + schedule[i][1] + schedule[i][2]
					+ stdTax - annPayment;
	...
	public void buildClat2Table() {
		...
		for (int i = 1; i < tableLength; i++) {
			...
			if (annuityType == 0) { // End of period payment
				toCharity[i] = toCharity[i - 1]
						+ (toCharity[i - 1] * charityGrowth);
				if (i <= term)
					toCharity[i] += annPayment;
			} else { // Begining of period Payment
				if (i <= term) {
					toCharity[i] = (toCharity[i - 1] + annPayment)
							+ ((toCharity[i - 1] + annPayment) * charityGrowth);
				} else {
					toCharity[i] = toCharity[i - 1]
							+ ((toCharity[i - 1]) * charityGrowth);
				}
	...
	public void buildStdTable() {
		...
		for (int i = 1; i < tableLength; i++) {
			...
			stdTable[i][4] = stdTable[i - 1][4] + stdTable[i][1]
					+ stdTable[i][2] + stdTable[i][3];
	...
	public void insert() {
		...
		if (error == 0) {
			uuid = dbObj.getUUID();
			record = dbObj.execute("select ID from CLAT_TOOL where UUID='"
					+ uuid + "'");
