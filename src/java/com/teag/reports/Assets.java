...
public class Assets extends Page {
	...
	private float businessInterests(float startTop) {
		...
		style = new String("[colspan=2,border=l]" + "[]"
				+ "[align=center,border=l+b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b+r]");
		...
		String sqlStatement = "select * from BUSINESS a, OWNERSHIP b, TITLE c, BUSINESS_TYPES d where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id and d.BUSINESS_TYPES_ID=a.BUSINESS_TYPE_ID";
		...
		while (rec != null) {
			...
			rowInfo[10] = dAnnualIncome == 0 ? "" : number
					.format(dAnnualIncome);
			...
			if ((r.getBottom() - tallest) < prctFull.getBottom()) {
				...
				goNewPage();
				top = assetHeader(h1, "Business Interests (cont.)", prctFull
						.getTop());
				rct.setTop(top);
		...
		// Do totals
		style = new String("[colspan=5,border=l+b]" + "[]" + "[]" + "[]" + "[]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]" + "[border=b]"
				+ "[bold=1,align=right,border=b]" + "[border=b]"
				+ "[border=b+r]");
	...
	private float cashAndEquivalents(float startTop) {
		...
		char ltr = 'a';
		dbObj.start();
		String sqlStatement = "select * from CASH a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		HashMap<String,Object> rec = dbObj.execute(sqlStatement);
		int i = 0;
		while (rec != null) {
			rowInfo[0] = "" + ltr + ".";
			ltr++;
			// Now grab all the pertinent numbers
			daBasis = rec.get("BASIS") == null ? 0 : ((Number) (rec
					.get("BASIS"))).doubleValue();
			dBasisSum += daBasis;
			daBalance = rec.get("VALUE") == null ? 0 : ((Number) (rec
					.get("VALUE"))).doubleValue();
			daInterRate = rec.get("INTEREST_RATE") == null ? 0 : ((Number) (rec
					.get("INTEREST_RATE"))).doubleValue();
			...
			if ((r.getBottom() - tallest) < prctFull.getBottom()) {
				// we need to go to a new page.
				goNewPage();
				top = assetHeader(h1, "Cash & Equivalents (cont.)", prctFull
						.getTop());
				rct.setTop(top);
		...
		totals[6] = dAnnualIncomeSum == 0 ? "" : number
				.format(dAnnualIncomeSum);
	...
	private int countRows(String table, long ownerId) {
		String sqlStatement = "select count(*) as x from " + table
	...
	private float debt(float startTop) {
		...
		char ltr = 'a';
		dbObj.start();
		String sqlStatement = "select * from DEBT a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		HashMap<String,Object> rec = dbObj.execute(sqlStatement);
		int i = 0;
		while (rec != null) {
			...
			daInterRate = rec.get("INTEREST_RATE") == null ? 0 : ((Number) (rec
					.get("INTEREST_RATE"))).doubleValue();
			int daYear = rec.get("LOAN_TERM") == null ? 0 : ((Number) (rec
					.get("LOAN_TERM"))).intValue();
			dBalanceSum += daBalance;
	...
	private float familyGifting(float startTop) {
		...
		GiftBean giftBean[] = gift.query(gift.OWNER_ID, Integer
				.toString(clientID));
		...
		style = new String("[align=center,border=l]"
				+ "[align=center,border=l+b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]");
	...
	private float illiquidInterest(float startTop) {
        ...
        String sqlStatement = "select count(*) as x from ILLIQUID where OWNER_ID='"
        ...
        sqlStatement = "select * from ILLIQUID a, OWNERSHIP b, TITLE c where OWNER_ID='"
	...
	private float lifeInsurance(float startTop) {
		...
		// Style for Header
		style = new String("[colspan=2,border=l,align=center]" + "[]"
				+ "[align=center,border=l+b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
		...
		InsuranceBean[] iList = ib.query(InsuranceBean.OWNER_ID, Long
				.toString(clientID));
		...
		if (iList != null) {
			for (int j = 0; j < iList.length; j++) {
				...
				if ((r.getBottom() - tallest) < prctFull.getBottom()) {
					// we need to go to a new page.
					goNewPage();
					top = assetHeader(h1, "Life Insurance (cont.)", prctFull
							.getTop());
					rct.setTop(top);
					r = this.columnHeader(header, widths, rct, colCount);
					i = 0;
		...
		// Do totals
		style = new String("[colspan=4,border=l+b]" + "[border=b]"
				+ "[border=b]" + "[border=b]" + "[border=b]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]" + "[border=b]"
				+ "[border=b]" + "[border=b+r]");
	...
	private float marketableSecurities(float startTop) {
		...
		char ltr = 'a';
		dbObj.start();
		String sqlStatement = "select count(*) as x from SECURITIES where OWNER_ID='"
				+ clientID + "'";
		HashMap<String,Object> rec = dbObj.execute(sqlStatement);
		int nAC = ((Number) (rec.get("x"))).intValue();
		sqlStatement = "select count(*) as x from BONDS where OWNER_ID='"
				+ clientID + "'";
		rec = dbObj.execute(sqlStatement);
		nAC += ((Number) (rec.get("x"))).intValue();
		...
		sqlStatement = "select * from BONDS a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		rec = dbObj.execute(sqlStatement);
		int i = 0;
		while (rec != null) {
			...
			if ((r.getBottom() - tallest) < prctFull.getBottom()) {
				// we need to go to a new page.
				goNewPage();
				top = assetHeader(h1, "Marketable Securities (cont.)", prctFull
						.getTop());
				rct.setTop(top);
				r = columnHeader(header, widths, rct, colCount);
				i = 0;
		...
		sqlStatement = "select * from SECURITIES a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		rec = dbObj.execute(sqlStatement);
	...
	private float notes(float startTop) {
		...
		String header[] = {
				...
				"Years",
				"Type",
				"[colspan=2,border=l][][align=center,border=l+b][align=center,border=b]"
						+ "[align=center,border=b][align=center,border=b][align=center,border=b][align=center,border=b+r]" };
		...
		char ltr = 'a';
		dbObj.start();
		String sql = "Select count(*) as x from NOTES where OWNER_ID='"
				+ clientID + "'";
		...
		double dYears = 0;
		double dSum = 0;
		sql = "select * from Notes a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.OWNERSHIP_ID=b.OWNERSHIP_ID and a.TITLE_ID=c.TITLE_ID";
		...
		while (rec != null) {
			...
			if (r.getBottom() - tallest < prctFull.getBottom()) {
				goNewPage();
				top = assetHeader(h1, "Notes Receivable (cont.)", prctFull
						.getTop());
				rct.setTop(top);
				r = columnHeader(header, widths, rct, colCount);
				i = 0;
	...
	private float notesPayable(float startTop) {
		...
		char ltr = 'a';
		dbObj.start();
		String sql = "Select count(*) as x from NOTE_PAYABLE where OWNER_ID='"
				+ clientID + "'";
		HashMap<String,Object> rec = dbObj.execute(sql);
		...
		double dYears = 0;
		double dSum = 0;
		sql = "select * from NOTE_PAYABLE a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.OWNERSHIP_ID=b.OWNERSHIP_ID and a.TITLE_ID=c.TITLE_ID";
		rec = dbObj.execute(sql);
		int i = 0;
	...
	private float personalUseProperty(float startTop) {
		...
		style = new String("[colspan=2,border=l]" + "[]"
				...
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b]"
				+ "[align=center,border=b]" + "[align=center,border=b+r]");
		...
		String sqlStatement = "select * from PROPERTY a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		HashMap<String,Object> rec = dbObj.execute(sqlStatement);
		int i = 0;
		while (rec != null) {
			...
			*/
			...
			if ((r.getBottom() - tallest) < prctFull.getBottom()) {
				// we need to go to a new page.
				goNewPage();
				top = assetHeader(h1, "Personal Use Property (cont.)", prctFull
						.getTop());
				rct.setTop(top);
				r = this.columnHeader(header, widths, rct, colCount);
				i = 0;
		...
		dbObj.stop();
		// Do totals
		style = new String("[colspan=4,border=l+b]" + "[]" + "[]" + "[]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]" + "[border=b]"
				+ "[border=b]" + "[border=b+r]");
	...
	private float realEstate(float startTop) {
		...
		String sqlStatement = "select * from REAL_ESTATE a, OWNERSHIP b, TITLE c where OWNER_ID='"
				+ clientID
				+ "' and a.ownership_id=b.ownership_id and a.title_id=c.title_id";
		...
		while (rec != null) {
			...
			dBasis = ((Number) (rec.get("BASIS"))).doubleValue();
			dLoanBal = ((Number) (rec.get("LOAN_BALANCE"))).doubleValue();
			dGrowth = rec.get("GROWTH_RATE") == null ? 0 : ((Number) (rec
					.get("GROWTH_RATE"))).doubleValue();
			dEquity = dMktValue - dLoanBal;
		...
		style = new String("[colspan=4,border=l+b]" + "[]" + "[]" + "[]"
				...
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]"
				+ "[bold=1,align=right,border=t+b]" + "[border=b+r]");
		String totals[] = { "", "", "", "", "", "", "", "", "", style };
		/*
	...
	private float retirementPlans(float startTop) {
		...
		for(RetirementBean retirement : rList){
			...
			rowInfo[3] = dAnnualContrib == 0 ? "" : number
					.format(dAnnualContrib); // annual contrib
			...
			if ((r.getBottom() - tallest) < prctFull.getBottom()) {
				// we need to go to a new page.
				goNewPage();
				top = assetHeader(h1, "Retirement Plans (cont.)", prctFull
						.getTop());
				rct.setTop(top);
				r = columnHeader(header, widths, rct, colCount);
				i = 0;
		...
		totals[3] = dAnnualContribSum == 0 ? "" : number
				.format(dAnnualContribSum);
