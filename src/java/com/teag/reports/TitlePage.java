...
public class TitlePage extends Page{
	...
	private String icons[] = {"queen.png",  "pawn.png", "knight.png", "castle.png","king.png"};
	private String labels[] = {"INTRODUCTION","CURRENT PLAN","PROPOSED PLAN","SUPPORTING\nDOCUMENTS","IMPLEMENTATION"};
	...
	public void draw()
	{
		...
		try
		{
			...
			float iconBase = (1.25f * 72);	// This is the base of the icons on the page
			...
			{
				icon[i] = Image.getInstance(Locations.getImageLocation() +  icons[i]);
			...
			for(int i = 0; i < icons.length; i++)
			{
				...
				icon[i].setAbsolutePosition((1.25f * 72) - iconLeft , rct.getTop() - iconBase);

				// Place the Icon
	            doc.add(icon[i]);            
	            
	            // Make adjustment to base for next icon, if on last icon skip it.
	            if(i < icons.length -1)
	            {
            ...
            iconBase = (1.25f * 72);	// This is the base of the icons on the page
            for(int i = 0; i < icons.length; i++)
            {
            	// Set the positioning
            	...
            	lblRct.setBottom(rct.getTop() - (iconBase - ((icon[i].getHeight() * scale) * .55f)));
            	Color color = new Color(0,0,0);
            	lbl.displayLabel(lblRct, labels[i], font, 7, color, Label.LBL_LEFT, Label.LBL_FIRST);
	            if(i < icons.length -1)
	            {
	            	height = icon[i+1].getHeight() * scale;
	            	iconBase += (height + (.25 * 72));
            ...
            lbl.displayLabel(lblRct, "PLANNING STRATEGIES",font,16, color ,Label.LBL_CENTER, Label.LBL_BOTTOM );
            
            color = new Color(0,0,0);
            lblRct.setBottom(doc.getPageSize().getTop() - (1.875f * 72));
            lbl.displayLabel(lblRct, "PREPARED FOR:", font, 13, color, Label.LBL_CENTER, Label.LBL_BOTTOM );
            
            color = new Color(0,69,119);
            lblRct.setBottom(doc.getPageSize().getTop() - (2.25f * 72));
            lbl.displayLabel(lblRct, this.client, font, 16, color, Label.LBL_CENTER, Label.LBL_BOTTOM );
            
            color = new Color(0,0,0);
            lblRct.setBottom(doc.getPageSize().getTop() - (2.625f * 72));
            lbl.displayLabel(lblRct, this.date, font, 13, color, Label.LBL_CENTER, Label.LBL_BOTTOM );
 
            color = new Color(0,0,0);

            lblRct.setBottom(doc.getPageSize().getTop() - (3.875f * 72));
            lbl.displayLabel(lblRct, "PRESENTED BY:", font, 12, color, Label.LBL_CENTER, Label.LBL_BOTTOM );
 
            /*
            BaseFont fontBold = PageUtils.LoadFont("timesbd.ttf");	// Need a bold font for the planner name
            lblRct.setBottom(doc.getPageSize().getTop() - (4.25f * 72));
            lbl.displayLabel(lblRct, this.planner, fontBold, 12, color, Label.LBL_CENTER, Label.LBL_BOTTOM );
            
            lbl.setLeadingAdjust(.75f);
            String FAX = this.fax.length() > 0 ? "\nFAX" + this.fax : "";
            String addr = this.address + "\nTEL " + this.telephone + FAX + "\n\n" + this.email;
            lblRct.setBottom(doc.getPageSize().getTop() - (4.5f * 72));
            lbl.displayLabel(lblRct, addr, font, 12, color, Label.LBL_CENTER, Label.LBL_FIRST );
            */
            ...
            lbl.displayLabel(lblRct, "The", font, 14, color, Label.LBL_LEFT, Label.LBL_TOP );
            
            lblRct.setLeft(lblRct.getLeft() + font.getWidthPoint("The ", 14));
            lbl.displayLabel(lblRct, "Estate", font, 30, color, Label.LBL_LEFT, Label.LBL_TOP );
            
            lbl.setLeadingAdjust(.60f);
            lblRct.setTop(doc.getPageSize().getTop() -(6.775f * 72));
            lblRct.setLeft(1.0625f * 72);
            lblRct.setRight(lblRct.getLeft() + font.getWidthPoint("Preservation ", 24));
            lbl.displayLabel(lblRct, "Overview", font, 24, color, Label.LBL_CENTER, Label.LBL_TOP );
            ...
            cb.setTextMatrix(crLeft, .375f * 72);	// Place the base of the copyright at 3/8" up from the bottom
...
//		Vector rows = new Vector();
		...
		String styles[] = {"[bold=1,align=center,"+colorStyle+"]", "[bold=1,align=center,valign=top,"+colorStyle+"]","[valign=top,align=center,"+colorStyle+"]"};
