...
public class EstatePlanTable extends EstatePlanSqlBean {
	...
	public void addSecurities(double value, int i) {
		double sRow[] = (double[]) cashFlowMap
				.get("Int & Div on Securities Portfolio");
		sRow[i] = value;
		...
		while (i.hasNext()) {
			...
			if (rValue > 0) {
				...
				if (nType.equalsIgnoreCase("I")) {
					...
					double payment = Function.PMT(rValue, nb.getInterestRate()
							/ nb.getPaymentsPerYear(), nb.getYears()
							* nb.getPaymentsPerYear())
							* nb.getPaymentsPerYear();
					for (int k = 0; k < nb.getYears(); k++) {
						sRow[k] += payment;
					}
	...
	public void buildClat() {
		if (clatValues.isGrantor()) {
			charitableDeduction.addDeduction(clatValues
					.getCharitableDeduction());
		}
		double sRow[] = new double[finalDeath + 1];
		for (int i = 0; i < finalDeath; i++) {
	...
	public void buildFamilyErp() {
		...
		while (itr.hasNext()) {
			...
			for (int i = 0; i < finalDeath; i++) {
				life[i] = cp.getClientDeathBenefit()
						+ cp.getSpouseDeathBenefit();
	...
	public void buildIditGrat() {
		double sRow[] = new double[finalDeath + 1];
		for (int i = 0; i < finalDeath; i++) {
			sRow[i] = iditValues.getGratPayment(i)
					+ iditValues.getNotePayment(i);
		}
	...
	public void buildLap() {
		...
		if (!lapList.isEmpty()) {
			...
			while (itr.hasNext()) {
				EstatePlanningToolBean lap = (EstatePlanningToolBean) itr
						.next();
		...
		while (itr.hasNext()) {
			...
			if (rValue > 0) {
				...
				if (n.getNoteType().equalsIgnoreCase("I")) {
					...
					double payment = Function.PMT(rValue, iRate
							/ paymentsPerYear, years * paymentsPerYear);
	...
	public void buildNotePayable() {
		...
		while (i.hasNext()) {
			...
			eab.setDbObject();
			double rValue = eab.getRemainingValue();
			NotePayableBean nb = (NotePayableBean) lab
					.loadAssetBean(eab);
			double iRate = nb.getInterestRate();
			String nType = nb.getNoteType();
			if (rValue > 0) {
	...
	public void buildRealEstate() {
		...
		while (i.hasNext()) {
			...
			if (rValue > 0) {
				...
				if (rb.getDepreciationValue() > 0
						&& rb.getDepreciationYears() > 0) {
	...
	public void calcIncomeTax(int year) {
		...
		double totalCash = getCRTotal(year) - getVcfTax(year)
				- patValues.taxFree[year] - taxDed[year];
		;
	...
	public double calcTax(double totalCash, int year) {
		...
		ccf.crutCapGains = wptValues.getValue(year)
				* (wptValues.payOutRate - wptValues.interestRate);
		ccf.stateIncomeTaxRate = cashFlowVars.getStateTaxRate();
		ccf.socialSecurity = socialSecurity.getSocialPayment(year);
		ccf.bonds = municipal.getBeginBalance(year)
				* municipal.getInterestRate();

		// We need to fix this, the CRT does not get a charitable deduction for
		// all of the years!
		double charity = cashFlowVars.getCharitableGifts()
				+ this.getVcfCharity(year) + charitableGift[year];

		charitableDeduction.addDeduction(charity);

		ccf.stateAGI = (totalCash) - ccf.socialSecurity - ccf.crutCapGains
				- ccf.bonds - depreciation[year];
	...
	public void eNetInflation() {
		double zRow[] = new double[finalDeath + 1];
		for (int i = 0; i < finalDeath; i++) {
			zRow[i] = (netToFamily[i] + taxableEstate[i])
					* Math.pow((1 + cashFlowVars.getInflation()), -i);
		}
	...
	public void eNetToCharity() {
		...
		double c3[] = (double[]) estateMap
				.get("PV of AMT to Charity from Test. CLAT");
	...
	public void eTaxableLife() {
		...
		while (i.hasNext()) {
			...
			double rValue = eab.getRemainingValue();
			if (rValue > 0) {
				InsuranceBean bb = (InsuranceBean) lab
						.loadAssetBean(eab);
	...
	public void eTClat() {
		...
		if (!tClatList.isEmpty()) {
			...
			estateMap.put("Asssets to " + Integer.toString(term)
					+ " Yr Test. Clat", tclat);
		}
	}
	...
	public void eTClatOld() {
		...
		if (useTClat) {
			...
			Iterator<EstatePlanningToolBean> itr = tClatList.iterator();
			while (itr.hasNext()) {
				EstatePlanningToolBean tclat = (EstatePlanningToolBean) itr
						.next();
			...
			estateMap.put("Assets to Test-Clat " + tclatValues.term
					+ " Yr Test. CLAT", zRow);
		}
	}
	...
	public double getClientGiftTax(int year) {
		...
		if (year == 0) {
			giftValue = gratCashFlow.getTaxableGift() / 2
					+ rpmValues.getToTrust(0) / 2
					+ mgtValues.getAssetValue() / 2;
		} else {
			giftValue += rpmValues.getToTrust(year) / 2;
		}
	...
	public void initVCF() {
		...
		if (vcf != null) {
			for (int i = 0; i < vcf.length; i++) {
				if (vcf[i].getUseFlag().equalsIgnoreCase("2")
						|| vcf[i].getUseFlag().equalsIgnoreCase("B")) {
