...
public class CashFlow extends AnalysisSqlBean {
		...
		*/
		...
		if (giftBean != null) {
			...
			for (int i = 0; i < giftBean.length; i++) {
				if (giftBean[i].getGiftPlanning().equals("B")
						|| giftBean[i].getGiftPlanning().equals("1")) {
					Date date;
					try {
	...
	public double getMortgage(int year) {
		double[] iTable = (double[]) dispMap
				.get("Real Estate Mortgage Principle Payments");
		return iTable[year];
	}

	public double getPersonalMortgage(int year) {
		double[] iTable = (double[]) dispMap
				.get("Personal Mortgage");
		return iTable[year];
	}
