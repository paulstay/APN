...
public class CashFlowTable extends AnalysisSqlBean {
	...
	public void secEnding() {
		...
		for (int i = 0; i < finalDeath; i++) {
			...
			double v = cashFlow.getSecurityBalance(i)
					+ cashFlow.getExcessCash(i) + cashFlow.getGrowth(i);
			CFCol c = new CFCol();
			if (v < 0) {
				c.setStrValue("" + formatNumber(v) + "");
				c.setFontColor(Color.red);
	...
	public void getIlliquid() {
		dbobj.start();
		String sql = "select * from ILLIQUID where owner_id='"
				+ cb.getPrimaryId() + "'";
		HashMap rec = dbobj.execute(sql);
		while (rec != null) {
			String description = (String) (rec.get("DESCRIPTION"));
	...
	public void getLiabilities() {
		...
		ArrayList<PropertyBean> pList = pb.getBeans("OWNER_ID='"
				+ cb.getPrimaryId() + "'");
	...
	public void getNotes() {
		...
		for (NotesBean n : notes) {
			...
			if (n.getNoteType().equalsIgnoreCase("I")) {
				...
				double payment = Function.PMT(n.getLoanAmount(), n
						.getInterestRate()
				...
				for (int i = 0; i < n.getYears(); i++) {
					for (int j = 0; j < n.getPaymentsPerYear(); j++) {
						double ci = balance
								* (n.getInterestRate() / n.getPaymentsPerYear());
	...
	public void getBusiness() {
		dbobj.start();
		String sql = "select * from Business where owner_id='"
				+ cb.getPrimaryId() + "'";
		HashMap rec = dbobj.execute(sql);
		while (rec != null) {
			String description = (String) (rec.get("DESCRIPTION"));
	...
	public void getPersonalProperty() {
		dbobj.start();
		String sql = "select * from PROPERTY where owner_id='"
				+ cb.getPrimaryId() + "'";
		HashMap rec = dbobj.execute(sql);
		while (rec != null) {
			String description = (String) (rec.get("DESCRIPTION"));
	...
	public void getTaxableEstate() {
		...
		for (int i = 0; i < finalDeath; i++) {
			CFCol c = new CFCol();
			double v = nw.getTotal(i) + rnd(estateLife[i]) - rnd(abEstate[i]);
			taxableEstate[i] = v;
			if (v > 0) {
				c.setStrValue(totalFormat(v));
			} else {
	...
	public void getTaxesAtDeath() {
		...
		for (int i = 0; i < finalDeath; i++) {
			...
			cet.setRpBalance(rp);
			cet.setTaxableEstate((nw.getTotal(i) * 1000) + estateLife[i]
					- abEstate[i]);
	...
	public void getInflation() {
		...
		CFRow r = new CFRow();
		r
				.setHeader("What's this really worth? - Inflation adjusted (3%) \nNet to Family");
		r.setIndentLevel(1);
		r.setTextFill(0);
		r.setFontWeight(Font.PLAIN);
		r.setFontColor(Color.green);
