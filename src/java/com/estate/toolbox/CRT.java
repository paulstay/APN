...
public class CRT {
	...
	public void calcCRTValues() {
		...
		for(int i=1; i < finalDeath; i++){
			...
			if(i < 6) {
				cDeduction = Math.min((agi+pay) * agiLimitation , cDed);
			...
			crt[i][3] = (pay - cDeduction)* taxRate;								// Tax
			crt[i][4] = pay - crt[i][3] - insurancePremium;		// netSpendable 
	...
	public void calcMax() {
		zCalc zc = new zCalc();
		zc.StartUp();
			maxPayout = zc.zCRUTMAX(0, clientAge, spouseAge, 0, 0, 0, irsRate, pmtPeriod, uniLag, 0, 0, 0);
			lifeExpectancy = zc.zLE(clientAge, spouseAge, 0, 0, 0, 0, 0);
		zc.ShutDown();
	}
	public void calcCrut(){
		zCalc zc = new zCalc();
		zc.StartUp();
			clientLe = zc.zLE(clientAge, 0, 0, 0, 0, 0, 0);
			spouseLe = zc.zLE(spouseAge, 0, 0, 0, 0, 0, 0);
			charitableDeductionFactor = zc.zUNITERMLIFE(2, payoutRate, 0, clientAge, spouseAge, 0, 0, 0, irsRate, pmtPeriod, uniLag, 0, 0, 0);
			optimalRate = zc.zCRUTMAX(0, clientAge, spouseAge, 0, 0, 0, irsRate, pmtPeriod, uniLag, 0, 0, 0);
		zc.ShutDown();
	}
	...
	public void calcOutRightSale() {
		...
		double pctIncome = taxIncome/payoutRate * investmentReturn;
		double pctGrowth = taxGrowth/payoutRate * investmentReturn;
		...
		for(int i=1; i < finalDeath; i++) {
			...
			noCrt[i][1] = (noCrt[i-1][3] * pctIncome * marginalTaxRate) + (noCrt[i-1][3] * pctGrowth * capitalGainsTax);
	...
	public boolean isRemainderTest() {
		return remainderTest;
	}

	public void setRemainderTest(boolean remainderTest) {
		this.remainderTest = remainderTest;
	}
