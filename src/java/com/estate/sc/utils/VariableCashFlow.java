...
public class VariableCashFlow {
	...
	static final int MAX_TABLE = 60;
	
	double vTable[] = new double[MAX_TABLE];
	...
	public void initialize() {
        ...
        double tValue = value;
        if( endingYear == 0)
        	endingYear = currentYear + MAX_TABLE + 5;
		for(int j = 0; j < MAX_TABLE; j++, currentYear++) {
	...
	public void setValues(VCFBean vcf) {
		...
		if( vcf.getTaxDeduction() != null && vcf.getTaxDeduction().equals("Y"))
			setTaxDed(true);
		else
			setTaxDed(false);
		initialize();
