...
public class LlcAssetFormServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5627097324248757379L;

	/**
	 * Constructor of the object.
	 */
	public LlcAssetFormServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// Create a LLCTool, or get the existing tool.
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, tool.getId(), ToolTableTypes.LLC.id());
				}
			}
		}
		...
		RequestDispatcher dispatch = request
				.getRequestDispatcher("/WEB-INF/jsp/estatetools/llc/assets.jsp");
