...
public class PatAssetServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 3732282593409553500L;

	/**
	 * Constructor of the object.
	 */
	public PatAssetServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String path = "/WEB-INF/jsp/estatetools/pat/assets.jsp";
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, pat.getId(), ToolTableTypes.PANNUITY.id());
				}
			}
		}
		...
		EPTAssetDistBean[] distributions = eptAssets.getToolDistributions(pat.getId(),
				ToolTableTypes.PANNUITY.id());
