...
public class RpmAssetServlet extends HttpServlet {

	private static final long serialVersionUID = 7242259755705638501L;

	/**
	 * Constructor of the object.
	 */
	public RpmAssetServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String path = "/WEB-INF/jsp/estatetools/rpm/assets.jsp";
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, rpm.getId(), ToolTableTypes.RPM.id());
				}
			}
		}
		...
		EPTAssetDistBean[] distributions = eptAssets.getToolDistributions(rpm.getId(),
				ToolTableTypes.RPM.id());
