...
public class QprtAssetServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4471540654208545950L;

	/**
	 * Constructor of the object.
	 */
	public QprtAssetServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String path = "/WEB-INF/jsp/estatetools/qprt/assets.jsp";
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, qprt.getId(), ToolTableTypes.QPRT.id());
				}
			}
		}
		...
		EPTAssetDistBean[] distributions = eptAssets.getToolDistributions(qprt.getId(),
				ToolTableTypes.QPRT.id());
