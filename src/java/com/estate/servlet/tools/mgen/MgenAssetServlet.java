...
public class MgenAssetServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -2138845304556987888L;

	/**
	 * Constructor of the object.
	 */
	public MgenAssetServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String path = "/WEB-INF/jsp/estatetools/mgen/assets.jsp";
		...
		if (action != null && action.equalsIgnoreCase("addAsset")) {
			...
			double percentDist = ValidatePercent.checkPercent(request
					.getParameter("v_dist"), 1.0);
			for (int i = 0; i < assetList.length; i++) {
				EPTAssetBean asset = assetList[i];
				if (assetId == asset.getAssetId()
						&& assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue() * percentDist,
							mgen.getId(), ToolTableTypes.MGEN.id());
				}
			}
		}
		...
		EPTAssetDistBean[] distributions = eptAssets.getToolDistributions(mgen
				.getId(), ToolTableTypes.MGEN.id());
		...
		for (int i = 0; i < distributions.length; i++) {
			...
			double basis = sb.getAssetBasis() * sb.getAssetValue()
					/ distributions[i].getValue();
			avb.setBasis(basis);
