...
public class FlpAssetFormServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, tool.getId(), ToolTableTypes.FLP.id());
				}
			}
		}
		...
		RequestDispatcher dispatch = request
				.getRequestDispatcher("/WEB-INF/jsp/estatetools/flp/assets.jsp");
