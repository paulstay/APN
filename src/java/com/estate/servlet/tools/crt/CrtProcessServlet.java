...
public class CrtProcessServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 5728000173260042227L;

	/**
	 * Constructor of the object.
	 */
	public CrtProcessServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (pageView != null && pageView.equals("save")) {
			if (crt.getId() < 0) {
				...
				aub.updateTools(crt.getId(), -epg.getScenarioId());
				EstatePlanningToolBean estate = new EstatePlanningToolBean();
				estate.setScenarioId(epg.getScenarioId());
				estate.setToolId(crt.getId());
				estate.setToolTableId(crt.getToolTableId());
				estate.setDescription("Charitable Remainder Trust");
				estate.insert();
			} else {
				crt.update();
			}
