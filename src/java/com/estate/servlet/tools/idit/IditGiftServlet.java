...
public class IditGiftServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -1657421412558225244L;

	/**
	 * Constructor of the object.
	 */
	public IditGiftServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		SIditTool tool = (SIditTool) session.getAttribute("sidit");
		...
		if(action != null && action.equalsIgnoreCase("addgift")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, tool.getId(), ToolTableTypes.SIDIT.id(),true);
				}
			}
		}
		
		if(action!= null && action.equalsIgnoreCase("delGift")) {
		...
		EPTAssetDistBean[] distributions = eptAssets.getToolDistributions(tool.getId(),
				ToolTableTypes.SIDIT.id());
		
		ArrayList<AssetViewBean> aList = new ArrayList<AssetViewBean>();
		for (int i = 0; i < distributions.length; i++) {
			if(distributions[i].isGift()){
		...
		RequestDispatcher dispatch = request
				.getRequestDispatcher("/WEB-INF/jsp/estatetools/idit1/gift.jsp");
