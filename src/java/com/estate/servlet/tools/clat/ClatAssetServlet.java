...
public class ClatAssetServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = -7440890868670155055L;

	/**
	 * Constructor of the object.
	 */
	public ClatAssetServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ClatTool tool = (ClatTool) session.getAttribute("clat");
		...
		if(action != null && action.equalsIgnoreCase("addAsset")){
			...
			for(int i=0; i < assetList.length; i++){
				...
				if(assetId == asset.getAssetId() && assetType == asset.getAssetType()) {
					asset.distribute(asset.getRemainingValue()* percentDist, tool.getId(), ToolTableTypes.CLAT.id());
				}
			}
		}
		...
		RequestDispatcher dispatch  = request.getRequestDispatcher("/WEB-INF/jsp/estatetools/clat/assets.jsp");
