...
public class DBObject implements Serializable {
    ...
    public String dbList(String field, String table, String where) {
        ...
        HashMap<String, Object> res = execute("select " + field + " from " + table + " where " + where);
        while (res != null) {
            if (list.length() > 0) {
                list.append(",");
            }
    ...
    public String formatDate(String date) {
        ...
        return datebegin + dateformat.parse(date, new ParsePosition(0)) + dateend;
    }
    ...
    public void initialize() {
        if (!initDb) {
            ...
            if (DBConstants.LOGGING) {
                ...
                String filename = DBConstants.LOGGING_PREFIX + df.format(new Date()) + ".sql";
        ...
        try {
            ...
            if (result.next()) {
                ...
                for (int i = 0; i < data.getColumnCount(); i++) {
                    ret.put(data.getColumnName(i + 1), result.getObject(data.getColumnName(i + 1)));
                }
            }
            m_uuid = "";
    ...
    public void setLoggingFilename(String filename) {
        ...
        if (log_filename != null) {
            ...
            if (DEBUG) {
                System.out.println("Database Logging to:" + file.getAbsolutePath());
            }
        }
    }
    ...
    public int start() {
        clearFields();
        if (isConnected()) {
            System.out.println("DBObject Something is wrong closing open connection");
    ...
    public int update() {
        ...
        try {
            ...
            for (int i = 0; i < fields.size(); i++) {
                sb.append((i == 0 ? "" : ",") + fields.get(i) + "=" + checkValue(values.get(i)));
            }
            sql = "update " + table + " set " + sb + " where " + whereclause;
            ResultSet res = stmt.executeQuery("SELECT * FROM " + table + " WHERE " + whereclause);
