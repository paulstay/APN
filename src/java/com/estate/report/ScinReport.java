...
/**
 * @author Paul Stay Description: Toolbox implementation of the SCIN,
 *         report..... copyright
 * @2008 Teag Software LLC.
 * 
 */
public class ScinReport extends Page {
	 ...
	 * @param writer
	 */
	public ScinReport(Document document, PdfWriter writer) {
	...
	private void page1() {
		...
		Rectangle rct;
		Utilities teagUtil = new Utilities();
		...
		drawHeader(clientHeading, "Self Canceling Installment Note");

		amount = scin.getFmv();
		basis = scin.getBasis();
		double le = teagUtil
				.getLifeExp(userInfo.getAge(), userInfo.getGender());
		...
		CellInfo cellData[][] = {

				{
						...
						cif.getCellInfo(dollar.format(amount),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
										ptSize, Font.NORMAL)) },

				{
						cif.getCellInfo("Less Cost(Basis)", new Font(
								baseFontBold, ptSize, Font.NORMAL)),
						cif.getCellInfo(dollar.format(basis),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
										ptSize, Font.NORMAL)) },

				{
						cif.getCellInfo("Taxable Gain", new Font(baseFontBold,
								ptSize, Font.NORMAL)),
						cif.getCellInfo(dollar.format(taxableGain),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
										ptSize, Font.NORMAL)) },

				{
						cif.getCellInfo("Income Tax Liability", new Font(
								baseFontBold, ptSize, Font.NORMAL, red)),
						cif.getCellInfo(dollar.format(capGainsTax),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
										ptSize, Font.NORMAL)) },

				{
						cif.getCellInfo("Net Proceeds", new Font(baseFontBold,
								ptSize, Font.NORMAL)),
						cif.getCellInfo(dollar.format(netProceeds),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
										ptSize, Font.NORMAL)) },

				{
						cif.getCellInfo("Estate Tax (" + percent.format(.55)
								+ ")", new Font(baseFontBold, ptSize,
								Font.NORMAL, red)),
						cif.getCellInfo(dollar.format(eTax),
								Element.ALIGN_RIGHT, new Font(baseFontBold,
	...
	public void page2() {
		...
		String theProcess[] = new String[] { "The self-cancelling installment note involves a sale of asset(s) to a family member in "
	...
	public void page3() {
		...
		String cells[][] = {
				...
				{ "Asset Value:", " ", " " + dollar.format(scin.getFmv()), " ",
						"7520 Rate", "" + percent.format(scin.getIrsRate()),
						"[][][][][align=left][]" },
				{ "Asset Basis", "", "" + dollar.format(basis), "",
						"7520 Date", "" + df.format(scin.getIrsDate()),
						"[][][][][align=left][]" },
				{ "Note Type", "", nType, "", "Note Term",
				...
				{
						"    Note Payment "
								+ (scin.getPaymentType() == 0 ? "(Principal Risk Premium)"
		...
		String ltHeader[] = {
				"Annual Cash Flow "
						+ (scin.getPaymentType() == 1 ? "(Principal Risk Premium)"
								: "(Interest Risk Premium)"), "", "", "", "",
		...
		for (int i = 0; i < scin.getTerm(); i++) {
			...
			data[1] = number.format(principal);
			data[2] = number.format(principal * growth);
			data[3] = number.format(payment[i]);

			principal = (principal + (principal * growth) - payment[i]);

			data[4] = number.format(principal);
			data[5] = "[align=center][align=right][align=right][align=right][align=right]";
			row[0] = data;
			leftTable.buildTableEx(row);
		}
		float lwidths[] = { .2f, .4f, .4f, .4f, .4f };
		int lalignments[] = { Element.ALIGN_LEFT, Element.ALIGN_LEFT,
				Element.ALIGN_RIGHT, Element.ALIGN_CENTER, Element.ALIGN_RIGHT };
	...
	public void rTable() {
		...
		for (int i = 0; i < scin.getTerm(); i++) {
			data[0] = Integer.toString(i + 1); // year
			data[1] = number.format(payment[i]);
			data[2] = number.format(interest[i]);
			data[3] = number.format(cap[i]);
			data[4] = number.format(taxFree[i]);
			data[5] = number.format(nBalance[i]);
