...
public class GratReport extends Page {
	...
	public GratReport(Document document, PdfWriter writer) {
		...
		if (isSingle) {
			...
			if (useLLC) {
				theProcess = new String[] {
						...
						"A Grantor Retained Annuity Trusts (GRATs) is established, for "
								+ userInfo.getFirstName()
								+ " and "
	...
	public void page3() {
		...
		CellInfo cellData[][] = {
...

				{
						...
						cif.getCellInfo(dollar.format(estateTaxGRAT) + " *",
								Element.ALIGN_CENTER, fontBRed),
						cif.getCellInfo(dollar
...

				{
						...
						cif.getCellInfo(dollar.format(netToFamilyGRAT),
								Element.ALIGN_CENTER, fontBGreen),
						cif.getCellInfo(dollar.format(Math.abs(netToFamilyGRAT
	...
	public void page5() {
		...
		rct.setTop(rct.getTop() + (1.5f * INCH));
