...
<html>
	...
	<body>
		...
		<jsp:useBean id="ht" scope="page" class="com.teag.bean.utils.HeadingText"/>
		<c:set var="inc" value="1" />
		<teag:form action="servlet/ClientRecommendationsServlet" method="post"
			name="aForm">
			...
			<table
				...
				<c:forEach var="s" items="${cList}">
					...
					<jsp:setProperty name="ht" property="text" value="${s.recommendation}"/>
					<tr class="${cclass}">
						<td width="60%">
							${inc}. ${ht.header}<br>${ht.body}
						</td>
						<td width='2%'>
							&nbsp;
						</td>
						<td width='10%' align='right' valign='top'>
							<a
								href="servlet/ClientRecommendationsServlet?action=Edit&id=${s.id}">Edit</a>&nbsp;
							<a
								href="servlet/ClientRecommendationsServlet?action=Delete&id=${s.id}">Delete</a>
						</td>
					</tr>
